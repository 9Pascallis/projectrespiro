          ...
          <table id="example2" class="table table-bordered table-hover mb-0">
              <thead>
                  <tr>
                      ...
                      @foreach ($processSewings as $processSewing)
                      <th style="width: 5%">{{ $processSewing->id }}</th>
                      @endforeach
              ...
              <tbody>
                  ...
                  <tr>
                      ...
                      @foreach ($processSewings as $processSewing)
                      <td>
                          @if (isset($data[$worksheet->id][$processSewing->id]))
                          {{ $data[$worksheet->id][$processSewing->id] }}
                          @else
                          -
                          @endif
                      </td>
                      ...
                      <td class="project-actions text-center">
                          <a class="btn btn-outline-info px" href="/input-sewing/{{ $worksheet->id }}"><i class="fas fa-plus"></i> </a>
                      </td>
                      <td class="project-actions text-center">
                          <a class="btn btn-outline-info px" href="/detail-sewing-output/{{ $worksheet->id }}"><i class="fas fa-eye"></i></a>
                      </td>
                  </tr>
                  @endforeach
              </tbody>
